...
class _SegmentedControlState<T> extends State<CustomSlidingSegmentedControl<T>>
  ...
  final Tween<FontWeight> _highlightTween =
      _FontWeightTween(begin: FontWeight.w300, end: FontWeight.w500);
  ...
  void didUpdateWidget(CustomSlidingSegmentedControl<T> oldWidget) {
    ...
    for (final oldKey in oldWidget.children.keys) {
    ...
    for (final newKey in widget.children.keys) {
  ...
  @override
  void dispose() {
    for (final animationController in _highlightControllers.values) {
      animationController.dispose();
    }

    for (final animationController in _pressControllers.values) {
      animationController.dispose();
    }
  ...
  void _animateHighlightController({T at, bool forward}) {
    if (at == null) return;
    final controller = _highlightControllers[at];
      ...
      builder: (BuildContext context, Widget child) {
        ...
        for (final currentKey in keys) {
          final textStyle = DefaultTextStyle.of(context).style.copyWith(
                fontWeight:
                    _highlightTween.evaluate(_highlightControllers[currentKey]),
                height: 1.23,
              );
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.all(
                Radius.circular(widget.trackRadius),
              ),
...
class _RenderSegmentedControl<T> extends RenderBox
  ...
  void _onDown(DragDownDetails details) {
    ...
    final index = indexFromLocation(_localDragOffset);
  ...
  void _onUpdate(DragUpdateDetails details) {
    _localDragOffset = details.localPosition;
    final newIndex = indexFromLocation(_localDragOffset);
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (highlightedIndex != null) {
      if (_childAnimations == null) {
        ...
        for (var i = 0; i < childCount - 1; i += 1) {
          ...
          var childAnimations = _childAnimations;
          childAnimations[child] =
              _ChildAnimationManifest(separatorOpacity: shouldFadeOut ? 0 : 1);
        }
      }

      final selectedChild = children[highlightedIndex];

      final childParentData =
          selectedChild.parentData as _SegmentedControlContainerBoxParentData;
      var kThumbInsets = _kThumbInsets;
      final unscaledThumbTargetRect =
          kThumbInsets.inflateRect(childParentData.offset & selectedChild.size);
      ...
      if (_needsThumbAnimationUpdate) {
        ...
        for (var i = 0; i < childCount - 1; i += 1) {
          ...
          final shouldFadeOut =
              i == highlightedIndex || i == highlightedIndex - 1;
          final child = children[i];
          final manifest = _childAnimations[child];
      ...
      for (var index = 0; index < childCount - 1; index += 1) {
  ...
  void _paintSeparator(
    ...
    final manifest = _childAnimations == null ? null : _childAnimations[child];
    final opacity =
    ...
    final separatorRect = _kSeparatorInset.deflateRect(
      childRect.topRight &
  ...
  bool hitTestChildren(BoxHitTestResult result, {@required Offset position}) {
    ...
    while (child != null) {
      ...
      if ((childParentData.offset & child.size).contains(position)) {
        final center = (Offset.zero & child.size).center;
